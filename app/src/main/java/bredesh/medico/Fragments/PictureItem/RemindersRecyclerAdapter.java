...
public class RemindersRecyclerAdapter extends RecyclerView.Adapter<RemindersRecyclerAdapter.CustomViewHolder> {
    ...
    public RemindersRecyclerAdapter(Context context, List<RemindersItem> remindersItems, Activity activity) {
    ...
    @Override
    public RemindersRecyclerAdapter.CustomViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.card_exercise_item, viewGroup, false);
        return new RemindersRecyclerAdapter.CustomViewHolder(view);
    }
...
//        customViewHolder.amount.setImageResource(R.drawable.ic_pill);
    ...
    private void activateAlerts(RemindersRecyclerAdapter.CustomViewHolder viewHolder, RemindersItem item) {
        int[] days = item.days;
